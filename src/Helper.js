...
export const specialsCheck = (day, pizzas, sundaySpecial) => {
  ...
  if (day === 0) {
    ...
    invoiceDetails.forEach((pizza) => {
      pizza.price =
        pizzaCount >= sundaySpecial.minQty
          ? sundaySpecial.pricePerPizza
          : pizza.regularPrice;
      pizza.pizzaDesc =
        pizzaCount >= sundaySpecial.minQty
          ? `${getWeekDayStr(0)} Special`
          : 'Regular';
...
export const checkEmptyInput = (text, setFeedbackMsg, msg) => {
  if (text.trim() === '') {
    if (setFeedbackMsg) {
      setFeedbackMsg(msg ? msg : 'Please, enter a valid input.');
    }
    return false;
  } else {
    setFeedbackMsg('');
